...
const SearchBox: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      try {
        ...
        auth.loggedInUser = userInfoResponse.data.result;
        setUserId(userInfoResponse.data.result._id);
  ...
  useEffect(() => {
    ...
    map.on('click', async function (e: LeafletMouseEvent) {
      ...
      const updatedCoordinates: Complaint = {
        _id: '',
        lat,
        lng,
        message: '',
    ...
    if (window.location.href.includes('/transportation')) setSelectedCategory('TRANSPORTATION');
    if (window.location.href.includes('/road')) setSelectedCategory('ROAD');
    if (window.location.href.includes('/healthcare')) setSelectedCategory('HEALTHCARE');
    if (window.location.href.includes('/security')) setSelectedCategory('DOMESTIC VIOLENCE AND CHILD ABUSE');
    if (window.location.href.includes('/education')) setSelectedCategory('EDUCATION');
    if (window.location.href.includes('/drinking-water')) setSelectedCategory('DRINKING WATER');
    if (window.location.href.includes('/crimes')) setSelectedCategory('CRIMES');
    if (window.location.href.includes('/domestic')) setSelectedCategory('DOMESTIC VIOLENCE AND CHILD ABUSE');
          ...
          <div style={{ marginTop: '10px', zIndex: 2 }}>
            ...
            <div>
              ...
              >
                ...
                <option value="EDUCATION">EDUCATION</option>
                <option value="CRIMES">CRIMES</option>
                <option value="DOMESTIC VIOLENCE AND CHILD ABUSE">DOMESTIC VIOLENCE AND CHILD ABUSE</option>
              </select>
